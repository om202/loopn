...
type DashboardSection = 'suggested' | 'connections' | 'saved' | 'search' | 'notifications' | 'account';
 ...
 */
export function DashboardSectionTracker({ activeSection }: DashboardSectionTrackerProps) {
  ...
  useEffect(() => {
    ...
    analytics.trackCustomEvent('dashboard_section_view', 'navigation', `dashboard_${activeSection}`);
...
function trackSectionSpecificEvents(section: DashboardSection, analytics: ReturnType<typeof useAnalytics>) {
  switch (section) {
    case 'suggested':
      analytics.trackCustomEvent('discover_section_loaded', 'discovery', 'suggested_users');
      break;
      
    case 'connections':
      analytics.trackCustomEvent('connections_section_loaded', 'networking', 'active_connections');
      break;
      
    case 'saved':
      analytics.trackCustomEvent('saved_section_loaded', 'favorites', 'saved_users');
      break;
      
    case 'search':
      analytics.trackCustomEvent('search_section_loaded', 'search', 'user_search');
      break;
      
    case 'notifications':
      analytics.trackCustomEvent('notifications_section_loaded', 'notifications', 'notification_center');
      break;
      
    case 'account':
      analytics.trackCustomEvent('account_section_loaded', 'account', 'user_account');
      break;
  }
}
