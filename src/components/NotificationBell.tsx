...
type NotificationFilter = 'all' | 'chat_request' | 'message' | 'connection' | 'system';
...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const subscription = chatService.observeChatRequests(
      ...
      async requests => {
        ...
        const newRequests = requestsWithUsers.filter(req => !previousRequestIds.includes(req.id));
        ...
        const chatNotifications: Notification[] = requestsWithUsers.map(request => ({
          id: request.id,
          type: 'chat_request' as const,
          title: request.requesterEmail || `User ${request.requesterId.slice(-4)}`,
    ...
    try {
      ...
      if (result.error) {
        ...
        const chatNotification: Notification = {
          id: chatRequest.id,
          type: 'chat_request',
          title: chatRequest.requesterEmail || `User ${chatRequest.requesterId.slice(-4)}`,
          content: 'wants to chat with you',
  ...
  const handleDialogAccept = () => {
    ...
    if (dialogRequest) {
      ...
      setNotifications(prev => prev.filter(notif => notif.id !== dialogRequest.id));
    }
  };
           ...
           <div className='p-4 border-b border-gray-100'>
             <div className='flex items-center justify-between'>
               ...
               {(() => {
                 ...
                 const activeTypes = Object.entries(counts).filter(([key, count]) => 
                   key !== 'all' && count > 0
                 ).length;
                   ...
                   <div className='relative'>
                     ...
                     >
                       ...
                       {counts.chat_request > 0 && (
                         <option value="chat_request">Chats ({counts.chat_request})</option>
                       )}
                       {counts.message > 0 && (
                         <option value="message">Messages ({counts.message})</option>
                       )}
                       {counts.connection > 0 && (
                         <option value="connection">Connections ({counts.connection})</option>
                       )}
                     </select>
                   </div>
                 ) : null;
            ...
            {error && (
              <div className='p-4 text-red-600 bg-red-50 text-sm'>
               ...
               <div className='p-8 text-center text-gray-500'>
                 ...
                 <p className='text-sm'>
                   {activeFilter === 'all' ? 'No notifications' : `No ${activeFilter.replace('_', ' ')} notifications`}
                 </p>
               </div>
             ) : (
               <div className='py-2'>
                       ...
                       <div className='flex-1 min-w-0'>
                         ...
                         {notification.type === 'chat_request' && (
                           <div className='flex gap-2 mt-1'>
                             <button
                               onClick={() =>
                                 handleRespondToRequest(notification.id, 'REJECTED', notification.data)
                               }
                               disabled={decliningId === notification.id || acceptingId === notification.id}
                             ...
                             >
                               {decliningId === notification.id ? 'Declining...' : 'Delete'}
                             </button>
                             <button
                               onClick={() =>
                                 handleRespondToRequest(notification.id, 'ACCEPTED', notification.data)
                               }
                               disabled={acceptingId === notification.id || decliningId === notification.id}
                             ...
                             >
                               {acceptingId === notification.id ? 'Accepting...' : 'Confirm'}
                             </button>
                           </div>
                         )}
                       </div>
