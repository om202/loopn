...
import { Play, Pause, RefreshCw, CheckCircle, AlertCircle, Users, Database } from 'lucide-react';
import { EmbeddingManager, UserProfileService } from '../../../services';
import type { BatchEmbeddingResult, EmbeddingJob, EmbeddingHealthCheck } from '../../../types/search.types';
...
export default function EmbeddingIndexerPage() {
  ...
  const [indexingResults, setIndexingResults] = useState<BatchEmbeddingResult | null>(null);
  const [healthCheck, setHealthCheck] = useState<EmbeddingHealthCheck | null>(null);
  ...
  const indexAllUsers = async () => {
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          const batchResult = await EmbeddingManager.batchProcessEmbeddings(batch);
          ...
          setIndexingProgress(prev => ({
            ...
            errors: [...prev.errors, ...batchResult.failed.map(f => `${f.userId}: ${f.error}`)]
          }));
          ...
          setIndexingProgress(prev => ({
            ...prev,
            processed: (batchIndex + 1) * batchSize,
            errors: [...prev.errors, `Batch ${batchIndex + 1} failed: ${batchError}`]
          }));
        }
      ...
      setIndexingProgress(prev => ({ ...prev, processed: jobs.length, current: undefined }));
      ...
      setIndexingProgress(prev => ({
        ...prev,
        errors: [...prev.errors, error instanceof Error ? error.message : 'Unknown indexing error']
      }));
    } finally {
      setIsIndexing(false);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please sign in to access the embedding indexer.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Embedding Indexer</h1>
          <p className="text-gray-600">
            Admin utility to generate embeddings for existing user profiles and enable RAG search functionality.
          </p>
        </div>
          ...
          {healthCheck && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{healthCheck.totalEmbeddings}</p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-600">
                  {healthCheck.lastUpdated ? new Date(healthCheck.lastUpdated).toLocaleString() : 'Never'}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <p className="text-gray-600 mb-4">
            This will process all existing user profiles and generate embeddings for RAG search functionality.
          ...
          {(isIndexing || indexingProgress.total > 0) && (
            <div className="mb-4">
              <div className="flex justify-between text-sm text-gray-600 mb-2">
                <span>
                  Progress: {indexingProgress.processed} / {indexingProgress.total}
          ...
          {indexingProgress.errors.length > 0 && (
            <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <h4 className="font-medium text-yellow-800 mb-2">Processing Errors:</h4>
              <div className="max-h-32 overflow-y-auto">
                <ul className="text-sm text-yellow-700 space-y-1">
                  ...
                  {indexingProgress.errors.length > 10 && (
                    <li className="text-yellow-600">... and {indexingProgress.errors.length - 10} more</li>
                  )}
                </ul>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div className="bg-green-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-green-600">{indexingResults.successful.length}</div>
                <div className="text-sm text-green-700">Successful</div>
              </div>
              <div className="bg-red-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-red-600">{indexingResults.failed.length}</div>
                <div className="text-sm text-red-700">Failed</div>
              </div>
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-blue-600">{indexingResults.metrics.totalProcessed}</div>
                ...
                <div className="max-h-32 overflow-y-auto">
                  <ul className="text-sm text-red-700 space-y-1">
                    {indexingResults.failed.slice(0, 10).map((failure, idx) => (
                      <li key={idx}>• {failure.userId}: {failure.error}</li>
                    ))}
                    {indexingResults.failed.length > 10 && (
                      <li className="text-red-600">... and {indexingResults.failed.length - 10} more</li>
                    )}
                  </ul>
                </div>
              </div>
