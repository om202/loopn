...
import { Search, User, Clock, Database, CheckCircle } from 'lucide-react';
import { RAGSearchService, EmbeddingService, EmbeddingManager } from '../../../services';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please sign in to test the search functionality.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">RAG Search Testing</h1>
          <p className="text-gray-600">
            Test the semantic search functionality with various queries to verify the RAG implementation.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          {/* Quick Test Queries */}
          <div className="mb-6">
            <h3 className="text-sm font-medium text-gray-700 mb-3">Quick Test Queries:</h3>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-blue-600">{searchResults.results.length}</div>
                <div className="text-sm text-blue-700">Results Returned</div>
              </div>
              <div className="bg-green-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-green-600">{searchResults.totalFound}</div>
                <div className="text-sm text-green-700">Total Matches</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-purple-600">{searchResults.metrics.totalProcessed}</div>
                <div className="text-sm text-purple-700">Profiles Processed</div>
              </div>
              <div className="bg-orange-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-orange-600">
                  {searchResults.metrics.queryEmbeddingTimeMs + searchResults.metrics.processingTimeMs + searchResults.metrics.fetchTimeMs}ms
                </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <h4 className="font-medium text-gray-800 mb-2">Performance Breakdown:</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <span className="text-gray-600">Query Embedding:</span>
                  <span className="font-medium ml-2">{searchResults.metrics.queryEmbeddingTimeMs}ms</span>
                </div>
                <div>
                  <span className="text-gray-600">Similarity Processing:</span>
                  <span className="font-medium ml-2">{searchResults.metrics.processingTimeMs}ms</span>
                </div>
                <div>
                  <span className="text-gray-600">Profile Fetching:</span>
                  <span className="font-medium ml-2">{searchResults.metrics.fetchTimeMs}ms</span>
                </div>
              </div>
            </div>
            ...
            {searchResults.results.length > 0 ? (
              <div className="space-y-4">
                {searchResults.results.map((result: SearchResult, idx: number) => (
                  <div key={result.userId} className="border border-gray-200 rounded-lg p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {result.profile.skills && result.profile.skills.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-3">
                            {result.profile.skills.slice(0, 5).map((skill: string, skillIdx: number) => (
                        ...
                        {result.embeddingText && (
                          <details className="mt-2">
                            <summary className="text-xs text-gray-500 cursor-pointer">Show matched text</summary>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  Try different search terms or check if embeddings have been generated for users.
                </p>
              </div>
            )}
          </div>
