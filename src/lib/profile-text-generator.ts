...
export function createProfileText(profile: any): string {
  ...
  const skipFields = [
    'userId', 'id', 'createdAt', 'updatedAt', '__typename',
    'profilePictureUrl', 'profilePictureThumbnailUrl', 'hasProfilePicture',
    'isOnboardingComplete', 'onboardingCompletedAt', 'autoFilledFields'
  ];
...
export function generateProfileVersion(profileData: any): string {
  ...
  const contentString = JSON.stringify(profileData, Object.keys(profileData).sort());
...
export function validateProfileText(profileText: string): {
  ...
  const wordCount = profileText.split(/\s+/).filter(word => word.length > 0).length;

  // Check minimum content requirements
  if (charCount < 50) {
    warnings.push('Profile text is very short - may not generate meaningful embeddings');
  }
  
  if (wordCount < 10) {
    warnings.push('Profile has very few words - consider encouraging users to fill more fields');
  }

  // Check maximum content (Titan v2 has 8K token limit)
  if (charCount > 8000) {
    warnings.push('Profile text exceeds 8000 characters - will be truncated for embedding');
  }

  // Check for meaningful content
  const uniqueWords = new Set(profileText.toLowerCase().split(/\s+/));
  if (uniqueWords.size < 5) {
    warnings.push('Profile has very few unique words - may not be distinctive for search');
  }

  const isValid = warnings.length === 0 || warnings.every(w => !w.includes('very short') && !w.includes('very few'));
...
export function testProfileTextGeneration(): void {
  const sampleProfile = {
    ...
    workExperience: JSON.stringify([
      { company: 'Startup Inc', role: 'Frontend Developer', duration: '2 years' },
