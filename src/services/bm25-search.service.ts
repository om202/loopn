...
export class BM25SearchService {
   ...
   */
  static buildIndex(profileTexts: Array<{ userId: string; text: string }>): void {
    ...
    const tokenizedDocuments = profileTexts.map(profile => 
      profile.text
        .toLowerCase()
  ...
  static updateDocument(userId: string, newText: string): void {
    ...
    const docIndex = BM25SearchService.documents.findIndex(doc => doc.userId === userId);
  ...
  static isExactTermQuery(query: string): boolean {
    ...
    if (words.length === 1 && words[0].length > 3) {
      ...
      const commonSemanticWords = [
        ...
        'architect', 'specialist', 'consultant', 'lead', 'senior',
        'experienced', 'skilled', 'expert', 'professional'
      ];
  ...
  static getIndexStats(): {
    ...
    return {
      ...
      avgDocLength: BM25SearchService.documents.length > 0 
        ? Math.round(
            BM25SearchService.documents.reduce((sum, doc) => sum + doc.text.length, 0) / 
