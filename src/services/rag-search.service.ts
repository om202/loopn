...
export class RAGSearchService {
  ...
  static async searchProfiles(
    ...
    try {
      ...
      const queryEmbedding = await EmbeddingService.generateEmbedding(searchOptions.query);
      ...
      if (embeddingChunks.length === 0) {
        ...
        return RAGSearchService.createEmptyResponse(searchOptions.query, queryEmbeddingTimeMs);
      }
      ...
      console.log(`Processed similarities in ${processingTimeMs}ms, found ${matchedChunks.length} matches`);
      ...
      if (topChunks.length === 0) {
        ...
        return RAGSearchService.createEmptyResponse(searchOptions.query, queryEmbeddingTimeMs, processingTimeMs);
      }

      // Step 5: Batch fetch real profile data for top matches only
      const fetchStart = Date.now();
      const results = await RAGSearchService.fetchProfileResults(topChunks, searchOptions);
      fetchTimeMs = Date.now() - fetchStart;

      console.log(`Fetched ${results.length} profile results in ${fetchTimeMs}ms`);
    ...
    for (const chunk of embeddingChunks) {
      try {
        ...
        if (!Array.isArray(vector) || vector.length !== 1024) {
          console.warn(`Invalid embedding vector for user ${chunk.userId}: expected 1024 dimensions, got ${vector?.length}`);
          continue;
        }
        ...
        console.error(`Failed to parse embedding for user ${chunk.userId}:`, parseError);
  ...
  private static calculateSimilarities(
    ...
    const matchedChunks: Array<{ userId: string; similarity: number; embeddingText?: string }> = [];
    ...
    for (const embedding of parsedEmbeddings) {
      ...
      try {
        ...
        if (similarity >= options.minSimilarity!) {
          const match: { userId: string; similarity: number; embeddingText?: string } = {
        ...
        console.error(`Similarity calculation failed for user ${embedding.userId}:`, similarityError);
  ...
  private static async fetchProfileResults(
    topChunks: Array<{ userId: string; similarity: number; embeddingText?: string }>,
    options: SearchOptions
    ...
    const profilePromises = topChunks.map(chunk => 
      client.models.UserProfile.get({ userId: chunk.userId })
        .catch(error => {
          console.error(`Failed to fetch profile for user ${chunk.userId}:`, error);
   ...
   */
  static async testSearch(testQuery: string = 'software engineer'): Promise<boolean> {
    try {
      const response = await RAGSearchService.searchProfiles(testQuery, { limit: 5 });
      
      console.log('Search service test results:', {
        query: testQuery,
        resultsCount: response.results.length,
        totalProcessed: response.metrics.totalProcessed,
        totalTime: response.metrics.queryEmbeddingTimeMs + response.metrics.processingTimeMs + response.metrics.fetchTimeMs
      });
